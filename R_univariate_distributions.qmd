...
The main parameter to play with when creating a histogram is the "width" of the bin.[^bincount] Run the code below to create a histogram with a bin basically for each value, but then play around with making the bin size larger to see how the visualization changes.

[^bincount]: Or alternatively, you can play with how many bins there are, but these are the same thing, in essence. I prefer to think about the width, because that is on the measurement scale. The number of bins is just an arbitrary decision about how many bars you'd like to see.
...
- `number_of_dice_rolled`: for changing the underlying distibution
- `number_of_rolls`: for simulating "more data collected", to see how much the distribution can vary when there's not very much data
- `histogram_bin_width`: for seeing how this changes the *visualization* of the data (instead of changing the data)
...
The `rnorm()` function does all the work here, and we just need to tell it how many values to generate, as well as the parameters.[^normdefaults]

[^normdefaults]: Quick quiz: what are the default values of `mean` and `sd` for the `rnorm()` function? How can you check? Why are these good choices for the defaults?
...
For reference, look back at the diagram of a normal distribution from the [concepts reading](univariate_distributions_reading.qmd#the-shape-of-the-normal) so you can see how these concepts of the mean ($\mu$) and standard deviation ($\sigma$) map to the distribution of data you are generating.
...
```{r}
ggplot(data = NULL, aes(x = normal_sample)) + geom_density()
...
We have already covered mean and median. Recall that the mean is the same as getting an *average* value,[^meancalc] and the median is the *middle* value, such that half of the data is higher and half is lower.[^mediancalc]

[^meancalc]: The sum of all values, divided by the number of values.

[^mediancalc]: And if there is no "middle" number because there's an even number of values, in R it's calculated as the average (mid-point) of the two middle numbers.
...
ggplot(data = NULL, aes(x = random_normals)) + geom_density() +
    ...
    geom_vline(aes(xintercept = random_normals_median), color = "blue", linetype = 2)
```

You can play around with these values, but as long as there are a lot
of numbers being generated, both the calculated mean and median will be quite close to
both the "theoretical" mean (the number we passed to the `mean` argument), as well as the mode (high point) of the distribution.
...
## Quantiles and the area under the curve
...
ggplot(data = NULL, aes(x = random_normals)) + geom_density() +
    ...
    theme_minimal() + 
    geom_polygon(data = density.025, aes(x = x, y = y), fill = "green")
```
...
Let's try out some of these things with a simple data set that is already in R, the `iris` data set.[^iris] Since the `iris` data comes pre-loaded, you don't have to read it in to use it. But since it's new to us, let's use some of the ways we've already seen to get some idea of what's in it.

[^iris]: This is a "classic" data set, which partly means that it gets used in a *lot* of tutorials. So apologies if you feel like you've already seen this data set too many times. Nevertheless, it's well-known and helps us illustrate some useful points, so it's still handy.
...
3. Next, generate the density values corresponding to the values in the variable, using the appropriate *density function*.
4. Then plot!
...
Finally, in order to plot the theoretical curve on top of the data, we need to do what we did above, plotting a histogram on the density scale, and then adding a layer for the theoretical densities we just computed.
...
The [final tutorial](R_transformations.qmd) you should check out in this Unit is much shorter, and it discusses the "how"s and "why"s of transforming variables, focusing on logarithmic transformations.
