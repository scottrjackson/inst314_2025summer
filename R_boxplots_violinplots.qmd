...
In this short guide, we will go over how to create a few basics plots to visualize data relationships when you have one continuous (or interval) variable, and one categorical variable.  This includes boxplots and violinplots.

We will also use this opportunity to examine how to use the `color` and `fill` properties in `ggplot2` graphs.

# Loading data and packages

We only need to load the `ggplot2` package.
...
A single boxplot is essentially a visualization of the quartiles of a variable. The "whiskers" (with any additional dots for "outlier" data) show the minimum and maximum values, and the "box" shows the other quartiles, where the lower edge of the box is the 25% quantile, the line in the middle of the box is the 50% quantile (aka median), and the upper edge of the box is the 75% quantile.

In the `sleep` data, the `extra` variable is continuous, and the `group` variable is categorical. We can use a boxplot to easily compare the difference in extra sleep between the groups. In `ggplot2`, we will still use `x` and `y` coordinates, just like in a scatterplot, but instead of `geom_point`, we can use `geom_boxplot`.

```{r}
ggplot(sleep, aes(x = group, y = extra)) + geom_boxplot()
```

The nice thing about boxplots is that they don't require you to aggregrate your data (like computing averages to put into bar plots), and they very quickly give you a visual comparison of values that are in different categories, including some distribution information.

## Limitations

However, boxplots also have a few limitations. They were actually invented by John Tukey as a technique for visualizing data *by hand*. That is, with just a pencil, some graph paper, and some numbers, you can construct a boxplot even without a computer. Because they were designed to be simple the drawback is that they convey very limited distributional information.
...
One possible solution is to plot density plots for each group. Fortunately, `ggplot2` gives us a powerful tool for plotting superimposed density plots by different groups.
...
We would like to modify this to show a different density plot for the two groups in the data, side-by-side in order to compare them. We can do that simply by assigning the color of the density plot to the `group` variable.
...
And here we have another example of overplotting, since the group 2 density is "covering up" part of the group 1 density. Returning to our strategies for handling overplotting, jittering doesn't make much sense here, but changing the alpha (opacity) does:
...
Violin plots are an attempt to essentially combine boxplots and density plots. What they show are density plots, but they are "mirrored" back-to-back for each group, and plotted in separate positions much like boxplots. Let's see what this look like for the sleep data:
...
Sometimes we might still prefer boxplots for their visual simplicity, but violin plots are a good additional tool to consider when you are visualizing differences between groups.

# Re-capping and reviewing with `iris`

Here we just show additional examples of these methods, using the `iris` data, which has more observations than the `sleep` data. In other words, looking at densities and violin plots when you only have 10 data points per group may be a little quetionable. There are 50 observations per Species in the iris data, which makes looking at densities a little more meaningful.
