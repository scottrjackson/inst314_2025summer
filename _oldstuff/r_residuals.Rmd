...
This notebook explains how to access and visualize the residuals and predicted values from a linear model in R. In the process, we will also discuss some about *list* objects in R, and that when you understand that many complex objects in R are structured similar to lists, it makes it easier to extract values from them.
...
The basic concept (discussed in the Unit 4 reading) is that when we have a fitted model, the regression line represents our predictions for the model, where we use *x* values to predict *y* values. The *difference* between a predicted *y* value and the actual *y* value observed for a data point is called the "error" or "residual." In this guide, I will henceforth call them "residuals", because that's the term most of the R functions and objects use.

The reading for Unit 4 goes into a little more detail about why we care about residuals. This guide is just here to show you how to work with them in R.
...
What you get is just a vector of numbers that is in the order of the data that was used to fit the model.  Easy.  This function may work for other kinds of models in R as well, if it's been implemented by the designers of those model objects.

## Getting residuals from the model object

However, the residuals are already stored in the model object itself. But to get them out, we need to talk a bit about complex objects and *list* objects in R.
...
- The items are still numbered starting from 1 like a vector, but there are double brackets `[[]]` instead of the typical single brackets. This means that we can get just the value "A" from the above list with `weird_list[[2]]`.
...
So it turns out that a LOT of objects in R share this basic property with lists, that they are collections of different things, bundled together into a single object. So this means that we can access the different parts of these objects just like we would access different parts of a list.
...
So what's in an "lm" object?  The first step is to use the `names` function:
...
As you can see, this output can be a little more overwhelming. But you can also see that there are the "dollar-sign" names at each level, so at the top we can see `$ coefficients`, `$ residuals`, and so on. These are the names we got with `names`, and by using the `$` notation, we can easily extract the values.
...
In Challenge 3, we got model predictions "the hard way," using the linear equation and our model parameters (intercept and slope), so that we could build understanding of how those predictions are generated.
...
Now that we know how to get residuals (and predicted values), we can look at how to visualize them. We can use the same tools we have learned in other Guides, namely histograms and scatterplots!
...
Is this somewhat redundant? Yes. If you have a huge amount of data, could this increase the data frame's size in memory too much?  Yes. But most of the time, it's a bit of a safer route to go, especially when you are still in the midst of exploring your data.

## Visualizing residuals step 1: histogram

Another side-benefit of making the residuals and predictions columns in your data is that it also makes it a little more convenient when we use `ggplot`, since we can just refer to the column names.
...
Refer to other Guides and resources for details on how to manipulate and further add to the histogram, like plotting a theoretical normal distribution curve to compare to the histogram shape. 
...
But the main takeaway here is that the data looks like kind of a random "cloud" centered vertically around 0 on the y axis.  This is what a "good" distribution of residuals when you have a linear regression.  The Code Practice and Challenge for Unit 4 will give you a look at what some "bad" or "degenerate" patterns of residuals can look like.
